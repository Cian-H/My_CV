...
#let muted      = rgb("#444444")   // medium gray — dates, locations, metadata
#let rule-color = rgb("#a0a0a0")   // light gray — hairline separators
...
#let section-heading(title) = {
  v(6pt)  // generous gap from previous section
...
#let cv-header(personal) = {
  ...
  align(center)[
    ...
    #if personal.at("scholar", default: "") != "" [
      #contact-sep
      #link(personal.scholar)[Scholar]
    ]
    #if personal.at("orcid", default: "") != "" [
      #contact-sep
      #link(personal.orcid)[ORCID]
    ]
    #if personal.at("website", default: "") != "" [
      #contact-sep
      #link(personal.website)[#personal.website.replace("https://", "")]
    ]
  ]
  v(1pt)
...
#let cv-skills(skills) = {
  block(breakable: true)[
    ...
    #for group in skills {
      block(width: 100%, breakable: false, inset: (left: 2pt, bottom: 2pt))[
        #text(size: body-size, weight: "bold", fill: body-color)[#group.category:]
        #h(5pt)
        #text(size: body-size, fill: body-color)[
          #group.items.join("  |  ")
        ]
      ]
    }
...
#let experience-entry(exp) = {
  block(width: 100%, breakable: false, inset: (left: 2pt, bottom: 1pt))[
    ...
    #for item in exp.highlights {
      block(inset: (left: 6pt, bottom: 0pt))[
...
// Publications
// ---------------------------------------------------------------------------
#let cv-publications(publications) = {
  block(breakable: true)[
    ...
    #for pub in publications {
      block(width: 100%, breakable: false, inset: (left: 2pt, bottom: 2pt))[
        #grid(
          ...
          column-gutter: 1em,
          align: (left, right),
          text(size: body-size, weight: "bold", fill: body-color)[#pub.title],
          text(size: small-size, fill: muted, style: "italic")[#pub.year],
        )
        #v(1pt)
        #text(size: small-size, fill: body-color)[
          #pub.journal
          #if pub.keys().contains("doi") and pub.doi != "" and pub.doi != none [ | #link("https://doi.org/" + pub.doi)[DOI] ]
          #if pub.keys().contains("url") and pub.url != "" and pub.url != none [ | #link(pub.url)[Link] ]
        ]
      ]
    }
  ]
...
// Service
// ---------------------------------------------------------------------------
#let cv-service(service) = {
  block(breakable: true)[
    #section-heading("Service & Leadership")
    #for s in service {
      block(width: 100%, breakable: false, inset: (left: 2pt, bottom: 2pt))[
        #grid(
          columns: (1fr, auto),
          ...
          align: (left, right),
          [
            #text(size: body-size, weight: "semibold", fill: body-color)[#s.role]
            #h(4pt)
            #text(size: small-size, fill: muted)[-- #s.organization]
          ],
          text(size: small-size, fill: muted)[#s.date],
        )
        #if s.keys().contains("description") and s.description != "" and s.description != none [
          #v(1pt)
          #text(size: small-size, fill: body-color)[#s.description]
        ]
      ]
    }
  ]
...
// Conferences
// ---------------------------------------------------------------------------
#let cv-conferences(conferences) = {
  block(breakable: true)[
    #section-heading("Conferences & Presentations")
    #for c in conferences {
      block(width: 100%, breakable: false, inset: (left: 2pt, bottom: 2pt))[
        #grid(
          ...
          align: (left, right),
          [
            #text(size: body-size, weight: "semibold", fill: body-color)[#c.name]
            #h(4pt)
            #text(size: small-size, fill: muted)[-- #c.role]
          ],
          text(size: small-size, fill: muted)[#c.date],
        )
      ]
    }
  ]
    ...
    #grid(
      ...
      ..languages.map(lang => {
        block(width: 100%, inset: (bottom: 2pt))[
          #text(size: body-size, weight: "semibold", fill: body-color)[#lang.name]
          #h(4pt)
          #text(size: small-size, fill: muted)[(#lang.proficiency)]
        ]
      })
    )
  ]
...
// ---------------------------------------------------------------------------
#let cv-education-and-certifications(education, certs) = {
  block(breakable: true)[
    ...
    #for edu in education {
      block(width: 100%, breakable: false, inset: (left: 2pt, bottom: 2pt))[
        ...
        #if edu.keys().contains("description") and edu.description != "" [
          #v(1pt)
          #text(size: small-size, fill: body-color)[#edu.description]
        ]
      ]
    }
    #for cert in certs {
      block(width: 100%, breakable: false, inset: (left: 2pt, bottom: 2pt))[
        #grid(
          ...
          align: (left, right),
          [
            #text(size: body-size, weight: "bold", fill: body-color)[#cert.name]
            #h(4pt)
            #text(size: small-size, fill: muted)[-- #cert.issuer]
          ],
          text(size: small-size, fill: muted)[#cert.year],
