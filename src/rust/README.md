...
This is the high-performance Rust backend for serving the CV data API. It reads directly from the SSOT HDF5 lockfile (`cv_data.h5`) and serves the data as JSON to the frontend.

## Building

The API must be built within the `devenv` shell to ensure it links correctly against the required HDF5 C headers:
...
You can run the server directly or via the python CLI orchestrator (`uv run cv serve`). 

The server is highly configurable to make deployment on your microserver easy. It supports both command-line arguments and environment variables.
